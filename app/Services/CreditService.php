...
class CreditService
{
    ...
    public function approveCredit(Credit $credit, array $decisionData): Credit
    {
        return DB::transaction(function () use ($credit, $decisionData) {
            ...
            return $credit->fresh(['status', 'payments']);
        });
    }

    public function rejectCredit(Credit $credit, int $processedBy): Credit
    {
        $status = CreditStatus::where('code', 'rejete')->firstOrFail();

        $credit->update(['credit_status_id' => $status->id, 'traite_par' => $processedBy]);
